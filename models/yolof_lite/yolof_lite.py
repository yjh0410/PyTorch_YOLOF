...
class YOLOF_Lite(nn.Module):
    def __init__(self, 
                 ...
                 trainable = False, 
                 topk = 1000):
        super(YOLOF_Lite, self).__init__()
        self.cfg = cfg
        self.device = device
        self.fmp_size = None
        ...
        ## neck
        self.neck = build_spp(cfg=cfg, in_dim=bk_dims[-1], out_dim=cfg['neck_dim'])
                                     
        ## head
        self.head = build_decoder(cfg, cfg['neck_dim'], num_classes, self.num_anchors)
